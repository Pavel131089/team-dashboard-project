...
import { getProjectsFromStorage, saveProjectsToStorage } from "@/utils/storageUtils";
...
export function useEmployeeData(navigate: NavigateFunction) {
  ...
  // Обработка проектов для получения задач
  const processProjects = (projectsList: Project[], userData: User): { 
    ...
    projectsList.forEach(project => {
      ...
      project.tasks.forEach(task => {
        ...
        const isAssigned = 
          ...
          (task.assignedToNames && task.assignedToNames.some(name => 
            name === userData.name || name === userData.username || name === userData.email
          ));
  ...
  // Обновление задачи
  const updateProjectTask = useCallback((projectId: string, updatedTask: Task): boolean => {
  ...
  const handleUpdateTaskProgress = useCallback((taskId: string, projectId: string, progress: number) => {
    try {
      ...
      const updatedTask: Task = {
        ...
        progress,
        actualStartDate: task.actualStartDate || new Date().toISOString(),
        actualEndDate: progress === 100 ? (task.actualEndDate || new Date().toISOString()) : (progress < 100 ? null : task.actualEndDate)
      };
